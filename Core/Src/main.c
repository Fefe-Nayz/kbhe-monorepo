...
/* USER CODE BEGIN PV */
uint16_t adc_buffer[2]; // [0]=CH10, [1]=CH12 (12-bit => 0..4095)
uint16_t adc_ch10_last = 0;
uint16_t adc_ch12_last = 0;
uint32_t num_conv = 0;
...
volatile uint32_t adc_dma_start_cycles = 0;
volatile uint32_t adc_dma_end_cycles = 0;
volatile uint32_t adc_dma_delta_cycles = 0;
volatile uint32_t adc_dma_delta_us = 0;

volatile uint32_t adc_dma_total_scan_start_cycles = 0;
volatile uint32_t adc_dma_total_scan_end_cycles = 0;
volatile uint32_t adc_dma_total_scan_us = 0;

volatile uint32_t adc_restart_start_cycles = 0;
volatile uint32_t adc_restart_end_cycles = 0;
volatile uint32_t adc_restart_us = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  MUX_SelectChannel(0);
  adc_dma_start_cycles = DWT->CYCCNT;
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
  if (hadc->Instance == ADC1) {
    num_conv++;
    adc_dma_end_cycles = DWT->CYCCNT;
    adc_dma_delta_cycles = adc_dma_end_cycles - adc_dma_start_cycles;
    adc_dma_delta_us = CyclesToUs(adc_dma_delta_cycles);

    adc_values[mux_channel] = adc_buffer[0];
    adc_values[NUM_MUX_CHANNELS + mux_channel] = adc_buffer[1];
    ...
    if (mux_channel >= NUM_MUX_CHANNELS) {
      // End of full scan
      adc_dma_total_scan_end_cycles = DWT->CYCCNT;
      adc_dma_total_scan_us = CyclesToUs(adc_dma_total_scan_end_cycles -
                                         adc_dma_total_scan_start_cycles);
      adc_dma_total_scan_start_cycles = DWT->CYCCNT;
    ...
    /**
     * Start 2us timer to delay next ADC start (after MUX settling time)
     */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
  if (htim->Instance == TIM4) {
    ...
    HAL_ADC_Start(&hadc1);  // Le DMA est déjà configuré en circulaire
  }
}
/* USER CODE END 4 */
